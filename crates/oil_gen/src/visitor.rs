...
use oil_ast::ast::{Import, MatchCase, Node, TraitNodeFn};
...
impl<'generator, 'input_key> ModuleVisitor<'generator, 'input_key> {
    ...
    pub fn visit_node(&mut self, node: &Node) {
        match node {
            ...
            Node::FnDeclaration { name, params, body } => {
                self.visit_fn_decl(name, params, body);
            }
            Node::AnFnDeclaration {
                ...
                self.visit_an_fn_decl(location, params, body);
            }
            Node::Break { location } => {
    ...
    /// Define variable
    fn visit_define(&mut self, name: &Token, value: &Node) {
        // value
    ...
    fn visit_fn_decl(&mut self, name: &Token, parameters: &Vec<Token>, body: &Node) {
        // params
    ...
    fn visit_an_fn_decl(&mut self, location: &Token, parameters: &Vec<Token>, body: &Node) {
        // params
