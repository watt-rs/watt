...
use fuel::bytecode::{Chunk, ModuleInfo};
use oil_analyze::analyzer::Analyzer;
use oil_ast::ast::{Import, Node};
use oil_common::{
    address::Address,
    error,
    errors::Error,
...
pub struct GeneratorResult {
    pub builtins: Chunk,
    pub main: Chunk,
    pub modules: FxHashMap<usize, ModuleInfo>,
}

/// Bytecode generator
pub struct BytecodeGenerator<'import_key> {
    modules: FxHashMap<usize, ModuleInfo>,
...
impl<'import_key> BytecodeGenerator<'import_key> {
    ...
    pub fn new() -> Self {
        BytecodeGenerator {
            ...
            libraries: HashMap::from([("std/io", PathBuf::from("./libs/std/std_io.oil"))]),
            builtins: PathBuf::from("./libs/base.oil"),
        }
    }
